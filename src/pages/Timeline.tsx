...
import { Plus, Clock, MapPin, Edit, Sparkles, Heart, GlassWater, Utensils, Music, Pizza } from "lucide-react";
...
const Timeline = () => {
  ...
  const [selectedEvent, setSelectedEvent] = useState<typeof timelineEvents[number] | null>(null);
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="flex gap-3">
            <Badge variant="outline" className="text-sm py-1.5 px-3">
              {events.length} Programmpunkte
            </Badge>
            <Badge variant="outline" className="text-sm py-1.5 px-3">
              {Math.floor(totalDuration / 60)}h {totalDuration % 60}min gesamt
            </Badge>
            ...
            {events.map((event, idx) => {
              ...
              return (
                <div key={event.id} className="relative">
                  {/* Dot */}
                  <div className="absolute left-6 md:left-1/2 -translate-x-1/2 w-4 h-4 rounded-full bg-champagne border-4 border-background z-10 mt-6" />
                  ...
                  <div className={cn(
                    ...
                    <button
                      ...
                      className="w-full text-left p-5 rounded-2xl bg-card border border-border/50 shadow-elegant hover:shadow-elevated transition-all group"
                    >
                      <div className="flex items-start gap-4">
                        <div className="w-11 h-11 rounded-xl bg-champagne-light flex items-center justify-center shrink-0">
        ...
        <div className="p-5 rounded-2xl bg-gradient-to-r from-champagne-light to-secondary border border-champagne/10 shadow-elegant">
            ...
            {events.map((e, i) => {
              ...
              return (
                <div key={e.id} className="flex items-center gap-2 shrink-0">
                  ...
                  <div>
                    <p className="text-xs font-medium whitespace-nowrap">{e.title}</p>
      ...
      <Sheet open={!!selectedEvent} onOpenChange={() => setSelectedEvent(null)}>
        <SheetContent>
          <SheetHeader>
            <SheetTitle className="font-serif">{selectedEvent?.title}</SheetTitle>
          </SheetHeader>
          {selectedEvent && (
            <div className="mt-6 space-y-6">
              <div className="space-y-3">
                {[
                  { label: "Uhrzeit", value: `${selectedEvent.time} – ${selectedEvent.endTime}` },
                  { label: "Ort", value: selectedEvent.location },
                ].map(item => (
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Button variant="outline" className="text-xs text-destructive hover:text-destructive">Löschen</Button>
              </div>
            </div>
          )}
        </SheetContent>
