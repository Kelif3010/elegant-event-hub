...
import { Plus, GripVertical, Trash2, Eye, Settings2, ArrowRight, ChevronDown, ChevronUp, Copy, ToggleLeft, X, Globe } from "lucide-react";
      ...
      case "radio":
        return (
          <div className="grid grid-cols-2 gap-3">
            {f.options?.map(opt => (
              <label key={opt} className="flex items-center gap-3 p-4 rounded-2xl border-2 border-border hover:border-champagne/40 cursor-pointer transition-all">
    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={addField} className="bg-champagne hover:bg-champagne/90 text-accent-foreground gap-2"><Plus className="h-4 w-4" /> Feld hinzufügen</Button>
          </div>
        </div>
            ...
            {sections.map(section => (
              <div key={section} className="p-6 rounded-2xl bg-card border border-border/50 shadow-elegant space-y-4">
                ...
                <div className="space-y-2">
                  {fields.filter(f => f.section === section).map((f, i) => {
                    const globalIdx = fields.indexOf(f);
                    return (
                      <div key={f.id} onClick={() => setEditField(f)} className={cn("flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer group", editField?.id === f.id ? "border-champagne bg-champagne-light/30" : "border-transparent hover:bg-secondary/40")}>
      ...
      <Sheet open={showPreview} onOpenChange={setShowPreview}>
        <SheetContent side="right" className="w-full sm:max-w-lg overflow-y-auto">
          <SheetHeader><SheetTitle className="font-serif">RSVP Vorschau</SheetTitle></SheetHeader>
          <div className="mt-6 space-y-6">
            {/* Step indicator */}
            <div className="flex items-center gap-2 justify-center">
              {previewSections.map((s, i) => (
                <div key={s} className="flex items-center gap-2">
                  <div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium", i <= previewStep ? "bg-champagne text-accent-foreground" : "bg-secondary text-muted-foreground")}>{i + 1}</div>
                  {i < previewSections.length - 1 && <div className={cn("w-6 h-px", i < previewStep ? "bg-champagne" : "bg-border")} />}
                </div>
              ))}
            </div>
            <p className="text-center text-sm font-medium">{previewSections[previewStep]}</p>
            <div className="p-6 rounded-2xl bg-card border border-border/50 space-y-5">
              {fields.filter(f => f.section === previewSections[previewStep]).map(f => (
            ...
            <div className="flex justify-between">
              ...
              {previewStep < previewSections.length - 1 ? (
                <Button onClick={() => setPreviewStep(previewStep + 1)}>Weiter <ArrowRight className="h-4 w-4 ml-1" /></Button>
              ) : (
                <Button className="bg-champagne text-accent-foreground">Absenden</Button>
              )}
            </div>
          </div>
        </SheetContent>
